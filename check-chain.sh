...
for i in `cat system-list.txt` 
	do
		echo "" | openssl s_client -showcerts -connect $i:443 2>&1 | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p;/-END CERTIFICATE-/a\\x0' | sed -e '$ d' | xargs -0rl -I% sh -c "echo '%' |  openssl x509 -subject -issuer -dates -fingerprint -MD5 -noout"  > tmp.cert
